all: merge_sections
	...
	pandoc -F pandoc-crossref -F ./graphviz.py -F ./comments.py --pdf-engine=xelatex doc-base/metadata.yaml --template=doc-base/eisvogel.latex -M date="`LC_ALL=en_US date "+%B %e, %Y"`" -H doc-base/yellowpaperstyle.pandoc yellow_paper.md --toc -o build/yellow_paper.pdf --bibliography doc-base/references.bib
...
section: merge_sections
	...
	[ -f sections/$(SECTION).md ] && pandoc -F pandoc-crossref -F ./graphviz.py -F ./comments.py --pdf-engine=xelatex sections/$(SECTION).yaml --template doc-base/eisvogel.latex -M date="`LC_ALL=en_US date "+%B %e, %Y"`" -H doc-base/yellowpaperstyle.pandoc sections/$(SECTION).md --toc -o build/$(SECTION).pdf --bibliography doc-base/references.bib || echo "Section not found"
