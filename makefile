all: merge_sections
	pandoc -F pandoc-crossref --pdf-engine=xelatex metadata.yaml --template=template.tex -M date="`LC_ALL=en_US date "+%B %e, %Y"`" -H yellowpaperstyle.pandoc yellow_paper.md --toc -o yellow_paper.pdf --bibliography references.bib
